...
beforeEach(() => {
  ...
  process.env = {
    ...OLD_ENV,
    REACT_APP_FEATURE_TELEPORTATION: "enabled",
    REACT_APP_FEATURE_TIME_TRAVEL: "disabled",
  }
})
...
describe("feature flags", () => {
  it("returns feature flags in environment variables", () => {
    expect(
      getFeatureFlags({ a: 1, REACT_APP_FEATURE_ROCKET_LAUNCH: "enabled" })
    ).toEqual({ ROCKET_LAUNCH: true })
  })
  it("returns feature flags in environment variables", () => {
    expect(
      getFeatureFlags({ a: 1, REACT_APP_FEATURE_ROCKET_LAUNCH: undefined })
