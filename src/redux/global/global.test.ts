...
import { BANNER_HIDDEN_TIMEOUT, initialGlobalState } from "./global.constants"
...
describe("global actions", () => {
  describe("when calling showBanner", () => {
    it("should open and close the banner after BANNER_HIDDEN_TIMEOUT", () => {
      ...
      jest.runTimersToTime(BANNER_HIDDEN_TIMEOUT)
      expect(store.getActions()).toEqual([
        _showBanner(testMessage_0, testTheme),
        _hideBanner(),
      ])
  ...
  describe("when connection fails to establish with daemon", () => {
    it("should fetch flow arguments from api.jina.ai", () => {
      ...
      const expectedAction = { type: "ARGUMENTS_FROM_API_FETCHED" }
  ...
  describe("when connection is successfully established with daemon", () => {
    it("should fetch flow arguments from daeomon", () => {
      const store = mockStore()
      const expectedAction = { type: "ARGUMENTS_FROM_DAEMON_FETCHED" }
